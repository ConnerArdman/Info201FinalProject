...
#TODO: style, comments, colorbar for counties, margins/sizing, conclusion info (mention excluded rows), DC?, name of panel
...
showState <- function(mill) {
  ...
  plot_geo(output.data, locationmode = 'USA-states') %>%
    add_trace(z = interest, text = text, locations = ~state, color = interest, colors = 'Reds', hoverinfo = 'text') %>%
